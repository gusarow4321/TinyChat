...
var (
	...
	prefixs = []string{"TINY_CHAT_MESSENGER_", "MESSENGER_"}
...
func newApp(logger log.Logger, gs *grpc.Server, hs *http.Server, k *kafka.ConsumerServer) *kratos.App {
	return kratos.New(
		kratos.ID(id),
		kratos.Name(Name),
...
func main() {
	...
	app, cleanup, err := wireApp(bc.Server, bc.Data, bc.Kafka, metricVecs, logger)
	if err != nil {
		panic(err)
	}
	defer cleanup()
